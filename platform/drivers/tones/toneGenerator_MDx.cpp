...
static uint32_t       toneTableIndex = 0; // Current sine table index for CTCSS
                                          // generator
static uint32_t toneTableIncr = 0;  // CTCSS sine table index increment per tick

static uint32_t beepTableIndex = 0; // Current sine table index for "beep"
                                    // generator
...
static uint8_t beepLockCount = 0; // Counter for management of "beep" generation
                                  // locking
...
void __attribute__((used)) TIM8_TRG_COM_TIM14_IRQHandler()
{
    ...
    {
        TIM3->CCR3 =
            (sineTable[(beepTableIndex >> 16) & 0xFF] * beepVolume) >> 8;
    }
