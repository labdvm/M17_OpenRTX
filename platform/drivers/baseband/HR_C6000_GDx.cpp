...
static const uint8_t initSeq2[] = {
    ...
    0x00, 0x00, 0x10, 0x70, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
void HR_Cx000<M>::init()
{
    ...
    writeReg(M::CONFIG, 0x0A, 0x01); // Clock connected to PLL, set Clock Source
                                     // Enable CLKOUT Pin
    ...
    writeReg(M::CONFIG, 0x10, 0x6E); // Set DMR, Tier2, Timeslot Mode, Layer 2,
                                     // Repeater, Aligned, Slot1
    writeReg(M::CONFIG, 0x11, 0x80); // Set LocalChanMode to Default Value
    writeReg(M::CONFIG, 0x13, 0x00); // Zero Cend_Band Timing advance
    writeReg(M::CONFIG, 0x1F, 0x10); // Set LocalEMB  DMR Colour code in upper 4
                                     // bits - defaulted to 1, and is updated
                                     // elsewhere in the code
    writeReg(M::CONFIG, 0x20, 0x00); // Set LocalAccessPolicy to Impolite
    writeReg(M::CONFIG, 0x21, 0xA0); // Set LocalAccessPolicy1 to Polite to
                                     // Color Code  (unsure why there are two
                                     // registers for this)
    ...
    writeReg(M::CONFIG, 0x2B, 0x0B); // According to Datasheet this is a Read
                                     // only register For FM Squelch
    writeReg(M::CONFIG, 0x2C, 0x17); // According to Datasheet this is a Read
                                     // only register For FM Squelch
    writeReg(M::CONFIG, 0x2D, 0x05); // Set FM Compression and Decompression
                                     // points (?)
    writeReg(M::CONFIG, 0x2E, 0x04); // Set tx_pre_on (DMR Transmission advance)
                                     // to 400us
    writeReg(M::CONFIG, 0x2F, 0x0B); // Set I2S Clock Frequency
    writeReg(M::CONFIG, 0x32, 0x02); // Set LRCK_CNT_H CODEC Operating Frequency
                                     // to default value
    writeReg(M::CONFIG, 0x33, 0xFF); // Set LRCK_CNT_L CODEC Operating Frequency
                                     // to default value
    writeReg(M::CONFIG, 0x34, 0xF0); // Set FM Filters on and bandwidth
                                     // to 12.5Khz
    ...
    writeReg(M::CONFIG, 0x37, 0x00); // Set mcu_control_shift to default. (codec
                                     // under HRC-6000 control)
    ...
    writeReg(M::CONFIG, 0x5F, 0xC0); // Enable Sync detection for MS or BS
                                     // orignated signals
    writeReg(M::CONFIG, 0x81, 0xFF); // Enable all Interrupts
    writeReg(M::CONFIG, 0xD1, 0xC4); // According to Datasheet this register is
                                     // for FM DTMF

    writeReg(M::CONFIG, 0x01, 0x70); // set 2 point Mod, swap receive I and Q,
                                     // receive mode IF
    writeReg(M::CONFIG, 0x03, 0x00); // zero Receive I Offset
    writeReg(M::CONFIG, 0x05, 0x00); // Zero Receive Q Offset
    writeReg(M::CONFIG, 0x12, 0x15); // Set rf_pre_on Receive to transmit
                                     // switching advance
    writeReg(M::CONFIG, 0xA1, 0x80); // According to Datasheet this register is
                                     // for FM Modulation Setting (?)
    ...
    writeReg(M::CONFIG, 0xE0, 0x8B); // CODEC under MCU Control, LineOut2
                                     // Enabled, Mic_p Enabled, I2S Slave Mode
    writeReg(M::CONFIG, 0xE1, 0x0F); // Undocumented Register (Probably
                                     // associated with CODEC)
    writeReg(M::CONFIG, 0xE2, 0x06); // CODEC  Anti Pop Enabled, DAC Output
                                     // Enabled
    writeReg(M::CONFIG, 0xE3, 0x52); // CODEC Default Settings
    writeReg(M::CONFIG, 0xE4, 0x4A); // CODEC   LineOut Gain 2dB, Mic Stage 1
                                     // Gain 0dB, Mic Stage 2 Gain 30dB
    writeReg(M::CONFIG, 0xE5, 0x1A); // CODEC Default Setting
    writeReg(M::CONFIG, 0x40, 0xC3); // Enable DMR Tx, DMR Rx, Passive Timing,
                                     // Normal mode
