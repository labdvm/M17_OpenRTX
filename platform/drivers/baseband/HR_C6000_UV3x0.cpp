...
static const uint8_t initSeq4[] = {0x01, 0x30, 0x00, 0x00, 0x20, 0x3C, 0xFF,
                                   0xFF, 0x3F, 0x50, 0x07, 0x60, 0x00, 0x00,
                                   0x00, 0x00, 0x00, 0x00
...
void HR_Cx000<M>::init()
{
    ...
    writeReg(M::CONFIG, 0x10, 0xF3); // FM mode, Tier II, TimeSlot, 3rd layer
                                     // mode, aligned (?)
    writeReg(M::CONFIG, 0x40, 0x43); // Enable RX synchronisation, normal mode
                                     // (no test)
    ...
    writeReg(M::CONFIG, 0x01, 0xF8); // Swap TX IQ, swap RX IQ, two point mode
                                     // for TX, baseband IQ mode for RX
    sendSequence(initSeq2, sizeof(initSeq2));
    writeReg(M::CONFIG, 0x00, 0x2A); // Reset codec, reset vocoder, reset I2S

    writeReg(M::CONFIG, 0x06, 0x20); // Vocoder output connected to universal
                                     // interface (?)
    ...
    writeReg(M::CONFIG, 0x1E, 0xF1); // Broadcast RX address, broadcast address
                                     // mask
    writeReg(M::CONFIG, 0xE2, 0x06); // Mic preamp disabled, anti-pop enabled
    writeReg(M::CONFIG, 0xE4, 0x27); // Lineout gain, first and second stage mic
                                     // gain
...
{
    writeReg(M::CONFIG, 0x10, 0x4F); // DMR mode, Tier I, TimeSlot, 2nd layer
                                     // mode, relay, aligned (?)
    writeReg(M::CONFIG, 0x81, 0x19); // Interrupt mask
    writeReg(M::CONFIG, 0x01, 0xF0); // Swap TX IQ, swap RX IQ, two point mode
                                     // for TX, IF mode for RX
    writeReg(M::CONFIG, 0xE4, 0x27); // Lineout gain, first and second stage mic
                                     // gain
    writeReg(M::CONFIG, 0xE5, 0x1A); // Undocumented register
    writeReg(M::CONFIG, 0x25, 0x0E); // Undocumented register
    ...
    writeReg(M::CONFIG, 0x41, 0x40); // Start RX for upcoming time slot
                                     // interrupt
    writeReg(M::CONFIG, 0x56, 0x00); // Undocumented register
    writeReg(M::CONFIG, 0x41, 0x40); // Start RX for upcoming time slot
                                     // interrupt
    writeReg(M::CONFIG, 0x5C, 0x09); // Undocumented register
    writeReg(M::CONFIG, 0x5F, 0xC0); // Detect BS and MS frame sequences in 2
                                     // layer mode
...
{
    writeReg(M::CONFIG, 0x10, 0x80); // FM mode, Tier II, TimeSlot, 3rd layer
                                     // mode, aligned (?)
    writeReg(M::CONFIG, 0x01, 0xB0); // Swap TX IQ, two point mode for TX, IF
                                     // mode for RX
    ...
    sendSequence(initSeq7, sizeof(initSeq7));
    writeReg(M::CONFIG, 0x11, 0x80); // Local channel mode
    writeReg(M::CONFIG, 0xE0, 0xC9); // Codec enabled, LineIn1, LineOut2, I2S
                                     // slave mode
...
void HR_Cx000<M>::startAnalogTx(const TxAudioSource source, const FmConfig cfg)
{
    ...
    // writeReg(M::CONFIG, 0xE2, 0x00);    // Mic preamp disabled, anti-pop
    // disabled
    ...
    writeReg(M::CONFIG, 0x25, 0x0E); // Undocumented Register
    writeReg(M::CONFIG, 0x26, 0xFE); // Undocumented register Turns off FM
                                     // receive
    ...
    writeReg(M::CONFIG, 0x3E, 0x08); // FM Modulation frequency deviation
