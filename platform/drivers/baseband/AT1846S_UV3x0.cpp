...
void AT1846S::init()
{
    ...
    i2c_writeReg16(0x1F, 0x1001); // Set gpio0 to ctcss_out/css_int/css_cmp
                                  // and gpio6 to sq, sq&ctcss/cdcss when
                                  // sq_out_set=1
    i2c_writeReg16(0x31, 0x0031);
    i2c_writeReg16(0x33, 0x44A5);
    i2c_writeReg16(0x34, 0x2B89);
    i2c_writeReg16(0x41, 0x4122); // Set voice_gain_tx (voice digital gain) to
                                  // 0x22
    i2c_writeReg16(0x42, 0x1052);
    i2c_writeReg16(0x43, 0x0100);
    i2c_writeReg16(0x44, 0x07FF); // Set gain_tx (voice digital gain after tx
                                  // ADC downsample) to 0x7
    i2c_writeReg16(0x59, 0x0B90); // Set c_dev (CTCSS/CDCSS TX FM deviation) to
                                  // 0x10 and xmitter_dev (voice/subaudio TX FM
                                  // deviation) to 0x2E
...
void AT1846S::setOpMode(const AT1846S_OpMode mode)
{
    if(mode == AT1846S_OpMode::DMR)
    {
        ...
        i2c_writeReg16(0x58, 0xBCFD); // Bit 0  = 1: CTCSS LPF bandwidth to
                                      // 250Hz Bit 3  = 1: bypass CTCSS HPF Bit
                                      // 4  = 1: bypass CTCSS LPF Bit 5  = 1:
                                      // bypass voice LPF Bit 6  = 1: bypass
                                      // voice HPF Bit 7  = 1: bypass
                                      // pre/de-emphasis Bit 11 = 1: bypass VOX
                                      // HPF Bit 12 = 1: bypass VOX LPF Bit 13 =
                                      // 1: bypass RSSI LPF
