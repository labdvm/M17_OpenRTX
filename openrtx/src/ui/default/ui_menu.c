...
static bool DidSelectedMenuItemChange(char *menuName, char *menuValue)
{
    ...
    if(strcmp(menuName, priorSelectedMenuName) != 0)
    {
        ...
        if(menuValue != NULL)
            ...
            *priorSelectedMenuValue = '\0'; // reset it since we've changed menu
                                            // item.
...
int _ui_getBankName(char *buf, uint8_t max_len, uint8_t index)
{
    ...
    else
    {
        ...
        if(result != -1) snprintf(buf, max_len, "%s", bank.name);
    }
    return result;
}
...
void _ui_drawSettingsVoicePrompts(ui_state_t *ui_state)
{
    ...
    _ui_drawMenuListValue(ui_state, ui_state->menu_selected,
                          _ui_getVoiceEntryName, _ui_getVoiceValueName);
}
...
bool _ui_drawMacroMenu()
{
    ...
    gfx_print(pos_2, layout.top_font, TEXT_ALIGN_CENTER, yellow_fab413, "5");
    char mode_str[12] = "";
    ...
    gfx_print(pos_2, layout.top_font, TEXT_ALIGN_RIGHT, yellow_fab413,
              "6        ");
    gfx_print(pos_2, layout.top_font, TEXT_ALIGN_RIGHT, color_white, "%.1gW",
              dBmToWatt(last_state.channel.power));
    // Third row
