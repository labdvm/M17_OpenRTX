...
const char *hwVersions[] = {"0.1d", "0.1e"};
...
int _ui_getMenuTopEntryName(char *buf, uint8_t max_len, uint8_t index)
{
    ...
    if(platform_getHwInfo()->hw_version < 1) maxEntries -= 1;
...
void _ui_drawSettingsM17(ui_state_t *ui_state)
{
    ...
    if(ui_state->edit_mode)
    {
        ...
        // uint16_t rect_height = (SCREEN_HEIGHT - (layout.top_h +
        // layout.bottom_h))/2; point_t rect_origin = {(SCREEN_WIDTH -
        // rect_width) / 2,
        //                        (SCREEN_HEIGHT - rect_height) / 2};
        // gfx_drawRect(rect_origin, rect_width, rect_height, color_white,
        // false); Print M17 callsign being typed
