...
typedef struct
{
    uint16_t buffer[VP_SEQUENCE_BUF_SIZE]; // Buffer of individual prompt
                                           // indices.
...
{
    if(vpDataLoaded == false) return;

#ifdef VP_USE_FILESYSTEM
    if(vpFile == NULL) return;
 ...
 *
 */
static bool GetSymbolVPIfItShouldBeAnnounced(
    char symbol, vpFlags_t flags, voicePrompt_t *vp)
{
    *vp = PROMPT_SILENCE;
...
static inline void disableSpkOutput()
{
    ...
    if((currentBeepDuration != 0) || (voicePromptActive == true)) return;
...
{
    if(currentBeepDuration > 0) platform_beepStop();
...
{
#ifdef VP_USE_FILESYSTEM
    if(vpFile == NULL) vpFile = fopen("voiceprompts.vpc", "r");

    if(vpFile == NULL) return;
#else
    if(&_vpdata_start == &_vpdata_end) return;
#endif
...
void vp_queueInteger(const int value)
{
    ...
    if(value < 0) vp_queuePrompt(PROMPT_MINUS);
...
void vp_queueStringTableEntry(const char *const *stringTableStringPtr)
{
    ...
    if(stringTableStringPtr == NULL) return;
...
void vp_tick()
{
    ...
    if(voicePromptActive == false) return;
    ...
    while(vpCurrentSequence.pos < vpCurrentSequence.length)
    {
        ...
        while(vpCurrentSequence.c2DataIndex < vpCurrentSequence.c2DataLength)
        {
            ...
            if(audioPath_getStatus(vpAudioPath) != PATH_OPEN) return;

            if(codec_pushFrame(c2Frame, false) == false) return;

            vpCurrentSequence.c2DataIndex += 8;
        }

        vpCurrentSequence.pos++; // ready for next prompt in sequence.
        vpCurrentSequence.c2DataLength = 0; // flag that we need to get more
                                            // data.
        vpCurrentSequence.c2DataIndex = 0;
    }
...
void vp_beep(uint16_t freq, uint16_t duration)
{
    ...
    if(currentBeepDuration != 0) return;
