...
{
    if(pos.x >= SCREEN_WIDTH || pos.y >= SCREEN_HEIGHT || pos.x < 0 ||
       pos.y < 0)
 ...
 */
static inline uint16_t get_line_size(
    GFXfont f, const char *text, uint16_t length)
{
 ...
 */
static inline uint16_t get_reset_x(
    textAlign_t alignment, uint16_t line_size, uint16_t startx)
{
    switch(alignment)
    {
...
// Print an error message to the center of the screen, surronded by a red (when
// possible) box
                      ...
                      uint32_t  active_sats)
{
    ...
    for(int i = 0; i < 12; i++)
    {
        ...
        color_t bar_color =
            (active_sats & 1 << (sats[i].id - 1)) ? yellow : white;
